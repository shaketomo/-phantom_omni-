...
public:
	...
	void force_callback(const phantom_omni::OmniFeedbackConstPtr& omnifeed) {
		...
		state->torque[0] = -omnifeed->torque.x;
		state->torque[1] = omnifeed->torque.y;
		state->torque[2] = omnifeed->torque.z;
	}
	...
	void publish_omni_state() {
		...
		if ((state->buttons[0] != state->buttons_prev[0])
			...
			if ((state->buttons[0] == state->buttons[1])
				...
				state->torque_control = !(state->torque_control);
				if(state->torque_control)
					ROS_INFO("Entering Torque Control Mode");
				else
					ROS_INFO("Leaving Torque Control Mode");
...
HDCallbackCode HDCALLBACK omni_state_callback(void *pUserData) {
	...
	//ROS_INFO("%f, %f, %f", omni_state->position[0], omni_state->position[1], omni_state->position[2]);
	...
	if (omni_state->lock == true) {
		//omni_state->lock_pos = omni_state->position;
	...
	}else if(omni_state->torque_control==true){
	  hdSetDoublev(HD_CURRENT_JOINT_TORQUE,omni_state->torque);
	
	  //ROS_INFO("Setting torque %f %f %f", omni_state->torque[0], omni_state->torque[1], omni_state->torque[2]);
